...
function solver_potential!(mesh)
    ...
    lens_pixels_area = zeros(Float64, height + 1, width + 1)
    ...
    return (image, prefix) -> begin
            ...
            if count % 1_000 == 1
                println("""
                    Iteration $(count) - max_update = $(round(new_update, sigdigits=4))
                        ...
                        $(field_summary(mesh.corners.ϕ - ϕ_b4, "ϕ changes since start of interations"))
        ...
        println("""
            Finished iteration at count $(count) - max_update = $(round(new_update, sigdigits=4))
                $(field_summary(mesh.corners.ϕ, "ϕ"))
                $(field_summary(mesh.corners.ϕ - ϕ_b4, "ϕ changes since start of interations"))
                $(field_summary(∇²ϕ_est, "∇²ϕ_est"))
                $(field_summary(δ, "δ"))
